...
func SetupTestDB() *sql.DB {
	db, err := sql.Open("mysql", "root@tcp(localhost:3306)/go-restapi-test")
...
func TestCreateCategorySuccess(t *testing.T) {
	...
	requestBody := strings.NewReader(`{"name":"film"}`)
	...
	assert.Equal(t, "film", responseBody["data"].(map[string]interface{})["name"])
}
